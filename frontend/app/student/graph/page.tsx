...
const TOPIC_GRAPH: Record<string, { name: string; prerequisites: string[]; subtopics: string[]; description: string }> = {
  linear_algebra:           { name: "Linear Algebra",           prerequisites: [],                                                          subtopics: ["Vectors", "Matrices", "Eigenvalues", "SVD"],                          description: "Foundation of all ML math — vectors, matrices, transformations." },
  calculus:                 { name: "Calculus",                 prerequisites: [],                                                          subtopics: ["Derivatives", "Gradients", "Chain Rule", "Optimization"],            description: "Derivatives and gradients power every learning algorithm." },
  programming_fundamentals: { name: "Programming Fundamentals", prerequisites: [],                                                          subtopics: ["Python", "Data Structures", "Algorithms", "OOP"],                    description: "Core coding skills needed to implement DS solutions." },
  probability_statistics:   { name: "Probability & Statistics", prerequisites: ["linear_algebra", "calculus"],                              subtopics: ["Distributions", "Bayes Theorem", "Hypothesis Testing", "MLE"],       description: "Mathematical framework for uncertainty and inference." },
  data_manipulation:        { name: "Data Manipulation",        prerequisites: ["programming_fundamentals"],                                subtopics: ["Pandas", "NumPy", "Data Cleaning", "Merging"],                       description: "Wrangling raw data into usable form with Python." },
  data_visualization:       { name: "Data Visualization",       prerequisites: ["data_manipulation"],                                       subtopics: ["Matplotlib", "Seaborn", "Plotly", "EDA"],                           description: "Communicating insights visually from data." },
  sql_databases:            { name: "SQL & Databases",          prerequisites: ["programming_fundamentals"],                                subtopics: ["Queries", "Joins", "Indexes", "Aggregations"],                      description: "Querying and managing structured data at scale." },
  statistical_learning:     { name: "Statistical Learning",     prerequisites: ["probability_statistics", "linear_algebra"],                subtopics: ["Regression", "Classification", "Regularization", "Cross-Val"],      description: "Statistical foundations of learning from data." },
  machine_learning:         { name: "Machine Learning",         prerequisites: ["statistical_learning", "data_manipulation"],               subtopics: ["Supervised", "Unsupervised", "Evaluation", "Ensemble"],             description: "Algorithms that learn patterns from data automatically." },
  feature_engineering:      { name: "Feature Engineering",      prerequisites: ["machine_learning", "data_manipulation"],                   subtopics: ["Encoding", "Scaling", "Selection", "Extraction"],                   description: "Transforming raw inputs into useful model features." },
  deep_learning:            { name: "Deep Learning",            prerequisites: ["machine_learning", "linear_algebra", "calculus"],          subtopics: ["Neural Nets", "Backprop", "CNNs", "RNNs"],                          description: "Multi-layer neural networks for complex representations." },
  computer_vision:          { name: "Computer Vision",          prerequisites: ["deep_learning"],                                           subtopics: ["Image Classification", "Object Detection", "Segmentation"],         description: "Teaching machines to understand images and video." },
  nlp:                      { name: "NLP",                      prerequisites: ["deep_learning", "probability_statistics"],                 subtopics: ["Tokenization", "Embeddings", "Transformers", "LLMs"],               description: "Processing and understanding human language with ML." },
  data_engineering:         { name: "Data Engineering",         prerequisites: ["sql_databases", "programming_fundamentals"],               subtopics: ["Pipelines", "Spark", "Airflow", "Data Lakes"],                      description: "Building infrastructure to move and store data reliably." },
  mlops:                    { name: "MLOps",                    prerequisites: ["machine_learning", "data_engineering"],                    subtopics: ["CI/CD", "Model Registry", "Monitoring", "Deployment"],              description: "Deploying and maintaining ML systems in production." },
};

// ── Color helpers ─────────────────────────────────────────────────────────────

function getMasteryColor(level: string, p_known: number): { base: string; glow: string } {
  if (level === "unassessed") return { base: "#2a2a3a", glow: "#494456" };
  if (p_known < 0.4)  return { base: "#1a0a2e", glow: "#7c3aed" };  // deep purple — beginner
  if (p_known < 0.7)  return { base: "#0a1a2e", glow: "#3b82f6" };  // blue — intermediate
  return { base: "#0a1f1a", glow: "#10b981" };                        // green — advanced
}
...
export default function KnowledgeGraphPage() {
  ...
  useEffect(() => {
    ...
    topicIds.forEach(id => {
      ...
      const colors = getMasteryColor(topic.level, topic.p_known);
    ...
    const animate = () => {
      ...
      nodes.forEach((node, i) => {
        ...
        mat.emissiveIntensity = 0.5 + Math.sin(t * 1.2 + i) * 0.2;
      });
  ...
  return (
    <>
      ...
      <div style={{ width: "100vw", height: "100vh", background: "radial-gradient(ellipse at center, #0f0818 0%, #050508 70%)", position: "relative", overflow: "hidden", fontFamily: "Manrope, sans-serif" }}>
        ...
        {/* ── Sidebar ── */}
        <Sidebar onCollapse={(c: boolean) => setSidebarWidth(c ? 60 : 252)} />
        ...
        <div style={{ position: "absolute", top: 0, left: sidebarWidth, right: 0, zIndex: 20, padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", background: "linear-gradient(to bottom, rgba(5,5,8,0.9), transparent)", transition: "left 0.25s cubic-bezier(0.16,1,0.3,1)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontSize: 16, fontWeight: 800, color: "#e2e2e2", letterSpacing: "-0.03em" }}>Knowledge Graph</span>
          </div>
          {/* Legend */}
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            {[
              { label: "Unassessed", color: "#494456" },
              { label: "Beginner",   color: "#7c3aed" },
          ...
          <div style={{
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 20, display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div>
                ...
                <div style={{ background: "#0f0f18", borderRadius: 12, padding: "14px 16px", border: "1px solid rgba(255,255,255,0.04)" }}>
                  {selectedTopic.level === "unassessed" ? (
                    <p style={{ color: "#333", fontSize: 13, margin: 0 }}>Not yet assessed — try <code style={{ color: "#6e28f5", background: "rgba(110,40,245,0.1)", padding: "1px 6px", borderRadius: 4 }}>/mcq {selectedTopic.name.toLowerCase()}</code></p>
                  ) : (
                    <>
              ...
              <div style={{ marginTop: "auto" }}>
                <button
                  onClick={() => router.push(`/student?mcq=${selectedTopic.id}`)}
